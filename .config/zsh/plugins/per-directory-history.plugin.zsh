...
function _per-directory-history-addhistory() {
  ...
  # respect hist_ignore_dups - skip if same as last command in file
  local last_in_file="$(tail -1 "$HISTFILE" 2>/dev/null)"
  if [[ "$cmd" != "$last_in_file" ]]; then
      print -Sr -- "$cmd"
      print -r -- "$cmd" >> "$HISTFILE"
      print -r -- "$cmd" >> "$_per_directory_history_directory"
  fi

  fc -p "$_per_directory_history_directory"
  # Return 1: keep in memory but don't let zsh write to file (we handle it)
  return 1
}
