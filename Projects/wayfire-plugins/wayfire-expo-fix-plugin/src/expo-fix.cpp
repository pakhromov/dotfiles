...
class wayfire_expo_fix : public wf::per_output_plugin_instance_t, public wf::keyboard_interaction_t,
    public wf::pointer_interaction_t, public wf::touch_interaction_t
{
  private:
    ...
    wf::option_wrapper_t<wf::color_t> background_color{"expo-fix/background"};
    wf::option_wrapper_t<wf::animation_description_t> zoom_duration{"expo-fix/duration"};
    wf::option_wrapper_t<int> delimiter_offset{"expo-fix/offset"};
    wf::option_wrapper_t<bool> keyboard_interaction{"expo-fix/keyboard_interaction"};
    wf::option_wrapper_t<double> inactive_brightness{"expo-fix/inactive_brightness"};
    wf::option_wrapper_t<int> transition_length{"expo-fix/transition_length"};
    ...
    workspace_bindings{"expo-fix/workspace_bindings"};
    ...
    std::unique_ptr<wf::expo_fix_wall_t> wall;
  ...
  public:
    ...
    void init() override
    {
        input_grab = std::make_unique<wf::input_grab_t>("expo-fix", output, this, this, this);

        setup_workspace_bindings_from_config();
        wall = std::make_unique<wf::expo_fix_wall_t>(this->output);
    ...
    void start_zoom(bool zoom_in)
    {
        ...
        wall->set_viewport_animating(true);
    ...
    void deactivate()
    {
        ...
        start_zoom(false);
        // The workspace is actually switched in finalize_and_exit(), when the exit animation
        // ends: switching it here damages every workspace stream (all views change their
        // wset-relative coordinates), which forces a re-capture of all workspace buffers right
    ...
    wf::effect_hook_t pre_frame = [=] ()
    {
        ...
        {
            wall->set_viewport_animating(false);
        }
    ...
    {
        state.active = false;
        if (output->wset()->get_current_workspace() != target_ws)
        {
...
class wayfire_expo_fix_global : public wf::plugin_interface_t,
    public wf::per_output_tracker_mixin_t<wayfire_expo_fix>
{
    wf::ipc_activator_t toggle_binding{"expo-fix/toggle"};
...
DECLARE_WAYFIRE_PLUGIN(wayfire_expo_fix_global);
