 ...
 */
struct expo_fix_wall_frame_event_t
{
    const wf::render_target_t& target;
    expo_fix_wall_frame_event_t(const wf::render_target_t& t) : target(t)
    {}
};
 ...
 */
class expo_fix_wall_t : public wf::signal::provider_t
{
  public:
    /**
     * Create a new workspace wall on the given output.
     */
    expo_fix_wall_t(wf::output_t *_output);
    ...
    /**
     * Indicate whether the viewport is currently being animated.
     *
     * While the viewport is animating, workspace buffers are kept at full resolution and are not
     * rescaled, so that animation frames only pay the cost of compositing the cached buffers.
     */
    void set_viewport_animating(bool animating);
