...
case "$1" in
  --preview)
    ...
    kitty +kitten icat --clear --transfer-mode=memory --unicode-placeholder --stdin=no \
      --place="${FZF_PREVIEW_COLUMNS}x${FZF_PREVIEW_LINES}@0x0" /tmp/cliphist-preview.png 2>/dev/null | sed $'$s/$/\e[m/'
    ;;
  --save)
    shift
    count=$#
  ...
  *)
    cliphist list | grep "\[\[ binary" | sed "s/\t\[\[ binary data /\t/; s/ \]\]$//" | fzf \
      ...
      --bind "delete:execute-silent($0 --delete {+1})+reload(cliphist list | grep \"\[\[ binary\" | sed \"s/\t\[\[ binary data /\t/; s/ \]\]$//\")" \
