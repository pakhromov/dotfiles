...
if [[ "$1" == "-t" ]]; then
  flag=/tmp/.cliphist_ok
  rm -f "$flag"
  $2 "$0"
  [ -f "$flag" ] && { rm -f "$flag"; wtype -M ctrl -k v; }
else
  sel=$(cliphist list | grep -v "\[\[ binary" | fzf \
    --bind "ctrl-delete:execute-silent($0 --delete {+1})+reload(cliphist list | grep -v \"\[\[ binary\")")
  if [ -n "$sel" ]; then
    content=$(echo "$sel" | cliphist decode)
    wl-copy -- "$content" >/dev/null 2>&1 && touch /tmp/.cliphist_ok
  fi
fi
