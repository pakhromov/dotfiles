...
_footer=$' \033[38;2;160;160;160mTAB\033[36m select   \033[38;2;160;160;160mCTRL+S\033[36m save   \033[38;2;160;160;160mCTRL+E\033[36m edit   \033[38;2;160;160;160mDEL\033[36m delete   \033[38;2;160;160;160mENTER\033[36m copy\033[m '
...
case "$1" in
  ...
  --save)
    ...
    count=$#
    for label in "$@"; do
      # cp is a byte copy, so the extension has to come from the real MIME type
      # or a jpeg/webp would land in ~/Pictures named .png. `get -l` only prints
      # MIME strings, so it is unaffected by the binary-output bug.
      mime=$(zzzclip get -l "$label" 2>/dev/null | head -1)
      ext="${mime#image/}"
